...
public class BanChar extends AdminCommand {
	...
	public void execute(Player admin, String... params) {
		...
		while (iter.hasNext()) {
			PacketSendUtility.sendBrightYellowMessageOnCenter(iter.next(), MuiService.getInstance().getMessage("CHAR") + playerName + MuiService.getInstance().getMessage("BAN1")+dayCount+MuiService.getInstance().getMessage("DAYS") + reason + "." + MuiService.getInstance().getMessage("GAVE") + admin + "");
		}
