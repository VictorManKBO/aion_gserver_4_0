	...
	public PlayerCommonData loadPlayerCommonData(final int playerObjId) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				WorldMap wm = World.getInstance().getWorldMap(worldId);
				if(wm.isInstanceType()) {
					InstanceExit exit = TeleportService2.getInstanceExit(worldId, cd.getRace());
					x = exit.getX();
					y = exit.getY();
					z = exit.getZ();
					heading = exit.getH();
					...
					MapRegion mr = wm.getMainWorldMapInstance().getRegion(x, y, z);
