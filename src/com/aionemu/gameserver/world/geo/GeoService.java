...
public class GeoService {
	...
	public float getZ(VisibleObject object) {
		float p1 = object.getZ();
		float p2 = geoData.getMap(object.getWorldId()).getZ(object.getX(), object.getY(), object.getZ(), object.getInstanceId());
		if (p2 == 0.0f || p1 > p2) {
			p2 = p1;
		}
		return p2;		
	...
	public float getZ(int worldId, float x, float y, float z, float defaultUp, int instanceId) {
		float p1 = z;
		float p2 = geoData.getMap(worldId).getZ(x, y, z, instanceId);
		if (p2 == 0.0f || p1 > p2) {
			p2 = z;
		}
