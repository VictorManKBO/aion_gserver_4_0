...
public class ItemService {
	...
	public static boolean checkRandomTemplate(int randomItemId) {
        ...
        public static void configuredItemByObjectId(final Player player, int itemObjId)
        {
            if(player == null)
                return;
            Storage storage = player.getInventory();
            if(storage == null)
                return;
            final Item item = storage.getItemByObjId(itemObjId);
            if(item != null && (item.isConfigured()))
                return;
            
            PacketSendUtility.broadcastPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(),
				item.getObjectId(), item.getItemTemplate().getTemplateId(), 5000, 0, 0), true);
		...
		public void run() {
                    ...
                    item.configureItem();
                    PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1401626, new DescriptionId(item.getNameId())));
