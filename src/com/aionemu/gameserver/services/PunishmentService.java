	...
	public static void setIsInPrison(Player player, boolean state, long delayInMinutes, String reason) {
		...
		if (state) {
			...
			if (delayInMinutes > 0) {
				...
				PacketSendUtility.sendMessage(player, MuiService.getInstance().getMessage("PRISON9") + delayInMinutes
					+ MuiService.getInstance().getMessage("PRISON10"));
			}
			...
			PacketSendUtility.sendMessage(player, MuiService.getInstance().getMessage("PRISON11"));
			
			if (GSConfig.ENABLE_CHAT_SERVER)
				PacketSendUtility.sendMessage(player, MuiService.getInstance().getMessage("PRISON12"));
	...
	public static void updatePrisonStatus(final Player player) {
		if (player.isInPrison()) {
			...
			if (prisonTimer > 0) {
				...
				PacketSendUtility.sendMessage(player, MuiService.getInstance().getMessage("PRISON13") + timeInPrison + MuiService.getInstance().getMessage("PRISON14")
					+ (timeInPrison > 1 ? MuiService.getInstance().getMessage("PRISON15") : "") + ".");

				player.setStartPrison(System.currentTimeMillis());
			}
			
			if (player.getWorldId() != WorldMapType.DF_PRISON.getId() && player.getWorldId() != WorldMapType.DE_PRISON.getId()) {
				PacketSendUtility.sendMessage(player, MuiService.getInstance().getMessage("PRISON16"));
